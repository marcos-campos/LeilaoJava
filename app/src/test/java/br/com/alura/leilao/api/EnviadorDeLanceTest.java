...
public class EnviadorDeLanceTest {
    ...
    @Test
    public void deve_MostrarMensagemFalha_QuandoLanceForMenorQueUltimoLance() {
        ...
        computador.propoe(new Lance(new Usuario("Alex"), 500.00));

        enviador.envia(computador, new Lance(new Usuario("Fran"), 100.00));

        verify(manager).mostraAvisoLanceMenorQueUltimoLance();
    }

    @Test
    public void deve_MostrarMensagemFalha_QuandoUsuarioComCincoLancesDerNovoLance() {
        EnviadorDeLance enviador = new EnviadorDeLance(
                client,
                listener,
                manager);

        doThrow(UsuarioJaDeuCincoLancesException.class).when(leilao).
                propoe(any(Lance.class));

        enviador.envia(leilao, new Lance(new Usuario("Alex"), 200.00));
    ...
    public void deve_MostrarMensagemDeFalha_QuandoOUsuarioDoUltimoLanceDerNovoLance(){
        ...
        enviador.envia(leilao, new Lance(new Usuario("Joao"), 150.00));
